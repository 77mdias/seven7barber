...
|----|-------------|--------|-------|
| REQ-SEC-01 | Availability race condition | ⚠️ Deferred | H7 - no booking service exists yet |
| REQ-SEC-02 | Timezone handling | ✅ Complete | UTC methods in availability.service.ts |
| REQ-SEC-03 | Voucher userId spoofing | ✅ Complete | vouchers.controller uses req.user.id |
| REQ-SEC-04 | Payment signature verification | ✅ Complete | HMAC-SHA256 with PAYMENT_WEBHOOK_SECRET |
| REQ-SEC-05 | OAuth token encryption | ⚠️ Deferred | H17 - better-auth manages tokens internally |
| REQ-SEC-06 | Auth response validation | ✅ Complete | response.ok checks in server/auth.ts |
| REQ-SEC-07 | Token refresh mechanism | ✅ Complete | POST /auth/refresh endpoint |
| REQ-SEC-08 | Rate limiting | ✅ Complete | @nestjs/throttler 5/min on auth |
